...
async def register_page(request: Request, ref: str = None):
    # نمرر كود الإحالة (ref) لصفحة التسجيل إذا كان موجوداً
...
async def register(
    ...
    phone: str = Form(...), 
    password: str = Form(...), 
    ref_code: str = Form(None), # الكود اللي جاي من الرابط
    db: Session = Depends(get_db)
):
    ...
    # عند إنشاء المستخدم الجديد، نستخدم ref_code لربطه بالمُدعي (Referrer)
