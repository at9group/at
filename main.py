from fastapi import FastAPI, Depends, Request, Form, responses, UploadFile, File
...
class Transaction(Base):
    ...
    image_path = Column(String, nullable=True) # تم إضافة عمود الصورة هنا
...
async def admin_panel(request: Request, db: Session = Depends(get_db)):
    ...
    if not is_admin:
        # إذا لم يكن مسجلاً، نرجع صفحة الأدمن فارغة أو تطلب التسجيل
...
async def create_withdraw(
    ...
    transfer_image: Optional[UploadFile] = File(None), # جعلناها اختيارية (None)
    db: Session = Depends(get_db)
):
    user = get_current_user(request, db)
    
    # التحقق من وجود الصورة
    if not transfer_image or transfer_image.filename == "":
        return responses.RedirectResponse(url="/withdraw?error=يجب رفع صورة التحويل لإتمام العملية", status_code=303)
    ...
    # حفظ الصورة بعد التأكد من وجودها
    upload_dir = "static/uploads"
    os.makedirs(upload_dir, exist_ok=True)
    file_path = f"{upload_dir}/{datetime.now().strftime('%Y%m%d%H%M%S')}_{transfer_image.filename}"
    with open(file_path, "wb") as buffer:
        shutil.copyfileobj(transfer_image.file, buffer)
        
    user.balance -= amount
    new_trans = Transaction(user_id=user.id, amount=amount, type="withdraw", status="pending", wallet=wallet, image_path=file_path)
    db.add(new_trans)
    db.commit()
    return responses.RedirectResponse(url="/profile?success=تم إرسال طلب السحب بنجاح", status_code=303)
...
@app.post("/deposit")
async def process_deposit(request: Request, amount: float = Form(...), image_path: UploadFile = File(...), db: Session = Depends(get_db)):
    user = get_current_user(request, db)
    if not user:
        return responses.RedirectResponse(url="/login")
        
    # 1. حفظ الصورة في مجلد static/uploads
    upload_dir = "static/uploads"
    os.makedirs(upload_dir, exist_ok=True)
    file_path = f"{upload_dir}/{datetime.now().strftime('%Y%m%d%H%M%S')}_{image_path.filename}"
    
    with open(file_path, "wb") as buffer:
        shutil.copyfileobj(image_path.file, buffer)
    
    # 2. حفظ المسار في قاعدة البيانات
    new_deposit = Transaction(user_id=user.id, amount=amount, type="deposit", status="pending", image_path=file_path)
    db.add(new_deposit)
    db.commit()
    
    return responses.RedirectResponse(url="/profile?success=تم إرسال طلب الإيداع", status_code=303)
...
async def register(
    ...
    phone: str = Form(...), 
    password: str = Form(...), 
    invite_code: str = Form(None), # تأكد أن الاسم هنا هو invite_code
    db: Session = Depends(get_db)
):
    ...
    # البحث عن المُدعي باستخدام كود الدعوة (invite_code)
    referrer = None
    if invite_code:
        referrer = db.query(User).filter(User.referral_code == str(invite_code)).first()
